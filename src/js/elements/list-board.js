...
class ListBoard extends Element {
  ...
  showChat(channelUrl, isLast) {
    this.chatRoom = ChatSection.createChatBoard(channelUrl, isLast);
    if (this.self.lastElementChild == this.listContent) {
      this.self.removeChild(this.listContent);
    }

    this._setContent(this.topTitle, 'Chat Board');
    this.self.appendChild(this.chatRoom);
  }
