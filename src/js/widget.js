...
import sendbirdModule from 'sendbird';
...
class SBWidget {
  ...
  _start(appId) {
    ...
    this.listBoard.addNewChatClickEvent(() => {
      console.log("addnewchatclickevent")
    ...
    this.listBoard.addChannelClickEvent(item, () => {
      if (channel.channelType !== 'noOp') {
        ...
        if (!openChatBoard) {
          ...
          channel.members.push(JSON.parse(localStorage.getItem('bhc_user')).user_id);

          channel.participants ?
              channel.participants.push(JSON.parse(localStorage.getItem('bhc_user')).user_id)
              :
              channel.participants = [JSON.parse(localStorage.getItem('bhc_user')).user_id];

          this._connectChannel(channelUrl, true, channel.channelType);
      }
      }
    });
    return item;
  ...
  showChannel(channelUrl) {
    ...
    this._connectChannel(channelUrl, false);
      console.log('after showchannel')
  }

  _connectChannel(channelUrl, doNotCall, channelType) {
    console.log("_connectChannel")
    var chatBoard = this.chatSection.createChatBoard(channelUrl, doNotCall, channelType);
    console.log("after create");
    if (!doNotCall) {
      this.responsiveChatSection(channelUrl, true);
    }
    this.chatSection.addClickEvent(chatBoard.closeBtn, () => {
      console.log("closed chatboard")
      //document.getElementById('sb_widget').style.display = 'block';
      this.chatSection.closeChatBoard(chatBoard);
      this.closePopup();
  ...
  getMessageList(channelSet, target, loadmore, scrollEvent) {
    let channelType = channelSet.channel.channelType;
    ...
    for (var i = 0; i < messageList.length; i++) {
      ...
      if (message.isTimeMessage && message.isTimeMessage()) {
        ...
        if (message.isAdminMessage()) {
          ...
          if (channel.channelType !== 'open') {
            unreadCount = channel.getReadReceipt(message);
          }
  ...
  getChannelSet(channel, isLast) {
    if (typeof channel === TYPE_STRING) {
      let tempChannel = this.listBoard.getChannelItem(channel)
      if (tempChannel.channelType === 'open') {
        channel = tempChannel
      }
    }
